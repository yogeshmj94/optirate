...
export function isBlockedAuctionSubmission(value: unknown): value is BlockedAuctionSubmission {
  if (typeof value !== 'object' || value === null) return false;
  const body = value as Record<string, unknown>;
  return (
    typeof body.completionRequestId === 'string' && body.completionRequestId.length > 0 && body.completionRequestId.length <= 128 &&
    typeof body.amount === 'number' && Number.isFinite(body.amount) && body.amount > 0 &&
    typeof body.tenureMonths === 'number' && Number.isInteger(body.tenureMonths) && body.tenureMonths > 0 &&
    typeof body.riskScore === 'number' && body.riskScore >= 0 && body.riskScore <= 100 &&
    body.riskAction === 'BLOCK_AUCTION' &&
    Array.isArray(body.riskReasons) && body.riskReasons.every((reason) => typeof reason === 'string')
  );
}
