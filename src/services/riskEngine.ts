...
export function buildMockCashflowHistory(sourceName = 'TATA_EMPLOYER_CORP_HASH'): Transaction[] {
  return Array.from({ length: 6 }, (_, index) => [
    fixtureTransaction(`salary-${index}`, `2025-${String(index + 1).padStart(2, '0')}-01T09:00:00.000Z`, 'INFLOW', 120000, sourceName, 'SALARY'),
    fixtureTransaction(`expense-${index}`, `2025-${String(index + 1).padStart(2, '0')}-05T09:00:00.000Z`, 'OUTFLOW', 42000, 'UTILITY_BILLER_HASH', 'BILL_PAYMENT'),
  ]).flat();
}

export function buildPanProfileTransactions(pan: string): Transaction[] {
  const profiles: Record<string, { inflow: number; outflow: number; source: string; chaotic: boolean; months: number }> = {
    ABCDE1234A: { inflow: 120000, outflow: 42000, source: 'TATA_EMPLOYER_CORP_HASH', chaotic: false, months: 6 },
    ABCDE1234B: { inflow: 160000, outflow: 60000, source: 'RANDOM_FI_SOURCE_HASH', chaotic: true, months: 6 },
    ABCDE1234C: { inflow: 90000, outflow: 120000, source: 'UNSEASONED_SOURCE_HASH', chaotic: true, months: 2 },
  };
  const profile = profiles[pan];
  if (!profile) return [];

  return Array.from({ length: profile.months }, (_, index) => {
    const month = String(index + 1).padStart(2, '0');
    const recurring = [
      fixtureTransaction(`pan-salary-${index}`, `2025-${month}-01T09:00:00.000Z`, 'INFLOW', profile.inflow, profile.source, 'SALARY'),
      fixtureTransaction(`pan-expense-${index}`, `2025-${month}-02T09:00:00.000Z`, 'OUTFLOW', profile.outflow, 'UTILITY_BILLER_HASH', 'BILL_PAYMENT'),
    ];
    return profile.chaotic
      ? recurring.concat(fixtureTransaction(`pan-leak-${index}`, `2025-${month}-03T09:00:00.000Z`, 'OUTFLOW', profile.inflow * 0.8, 'CASH_SINK_HASH', 'CASH_WITHDRAWAL'))
      : recurring;
  }).flat();
}
